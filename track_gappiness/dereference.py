...
def get_event_final_hits_packets(ff, event):
    events_hits_ref = ff['charge/events/ref/charge/calib_final_hits/ref']
    events_trig_ref = ff['charge/events/ref/charge/ext_trigs/ref']
    events_hits_rref = ff['charge/events/ref/charge/calib_final_hits/ref_region']
    events_trig_rref = ff['charge/events/ref/charge/ext_trigs/ref_region']
    
    hits = dereference(event, events_hits_ref, ff['charge/calib_final_hits/data'], region=events_hits_rref).flatten()
    ...
    packets = dereference(hits['id'], hits_packets_ref, ff['charge/packets/data'], region=hits_packets_rref).flatten()
    
    return hits, packets

def get_event_hits_packets(ff, event):
    ...
    hits = dereference(event, events_hits_ref, ff['charge/calib_prompt_hits/data'], region=events_hits_rref).flatten()
...
def get_event_trigs(ff, event):

    events_trig_rref = np.array(ff['charge/raw_events/ref/charge/ext_trigs/ref'][:])
